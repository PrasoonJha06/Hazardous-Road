...
# Explosive variables
explosive_surface = pygame.image.load('Explosive.png').convert()
explosive_rect = explosive_surface.get_rect(center=(random.randint(50, 425), random.randint(-50000, -10000)))
explosive_y_increment = 1
...
def redraw_explosive():
    """Bringing explosive back to game once it has gone out of the screen"""
    if explosive_rect.bottom >= 725:
        explosive_rect.x = random.randint(50, 425)
        explosive_rect.y = random.randint(-50000, -20000)


def accident():
    """Creating a scene of an accident"""
    global enemy_car_y_increment, truck_y_increment, explosive_y_increment, road_y_increment, score_increment
    if car_rect.y >= 500:
        enemy_car_y_increment = 0
        truck_y_increment = 0
        explosive_y_increment = 0
        road_y_increment = 0
        score_increment = 0
...
while active:
    ...
    explosive_rect.y += explosive_y_increment
    ...
    screen.blit(explosive_surface, explosive_rect)

    # Calling functions
    fix_car()
    detect_collision(enemy_car_rect)
    detect_collision(truck_rect)
    detect_collision(explosive_rect)
    redraw_enemy_car()
    redraw_truck()
    redraw_explosive()
    accident()
